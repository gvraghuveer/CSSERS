...
// EMERGENCY HANDLERS
// ===========================================
static esp_err_t emergency_on_handler(httpd_req_t *req) {
  emergencyMode = true;
  ...
  httpd_resp_set_type(req, "application/json");
  httpd_resp_set_hdr(req, "Access-Control-Allow-Origin", "*");
  return httpd_resp_send(req, "{\"status\":\"emergency_on\"}", HTTPD_RESP_USE_STRLEN);
}

static esp_err_t emergency_off_handler(httpd_req_t *req) {
  emergencyMode = false;
  ...
  httpd_resp_set_type(req, "application/json");
  httpd_resp_set_hdr(req, "Access-Control-Allow-Origin", "*");
  return httpd_resp_send(req, "{\"status\":\"emergency_off\"}", HTTPD_RESP_USE_STRLEN);
}

static esp_err_t emergency_status_handler(httpd_req_t *req) {
  char response[64];
...
void startCameraServer() {
  ...
  httpd_uri_t emergency_on_uri = {
    .uri = "/emergency/on",
    .method = HTTP_GET,
    .handler = emergency_on_handler,
    .user_ctx = NULL
  };

  httpd_uri_t emergency_off_uri = {
    .uri = "/emergency/off",
    .method = HTTP_GET,
    .handler = emergency_off_handler,
    .user_ctx = NULL
  };

  httpd_uri_t emergency_status_uri = {
    .uri = "/emergency/status",
    .method = HTTP_GET,
    .handler = emergency_status_handler,
    .user_ctx = NULL
  };
  ...
  if (httpd_start(&camera_httpd, &config) == ESP_OK) {
    ...
    httpd_register_uri_handler(camera_httpd, &emergency_on_uri);

    httpd_register_uri_handler(camera_httpd, &emergency_off_uri);

    httpd_register_uri_handler(camera_httpd, &emergency_status_uri);
  }
